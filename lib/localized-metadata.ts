...
export async function localizedMetadata(language:string,section:Section,dictionary:Record<string,string>,complete=true,root=false):Promise<Metadata>{
 const origin=originFromHeaders(await headers());const suffix=section==="home"?"":`/${section}`;const path=root?"/":`/${language}${suffix}`;
 ...
 const languages=Object.fromEntries([...LANG_ORDER.map(code=>[code,`${origin}/${code}${suffix}`]),["x-default",root&&section==="home"?origin:`${origin}/en${suffix}`]]);
